...
class DLPC350:
    ...
    def __init__(self, dlp_path, debug=0, dryrun=0):
        self.dlp_path = dlp_path
        self.debug = debug
        self.dryrun = dryrun
        self.seqN = 0x01
        self.connected = 0
    ...
    def connectDLP(self):
        try:
            self.dlp_hid.open_path(self.dlp_path)
            self.connected = 1
            if(self.debug):
    ...
    def loadImageToFlash(self, image_data, flash_index):
        '''Load an image into the DLPC350's flash memory.
        Parameters:
            image_data (bytes): The image data to load into flash memory.
            flash_index (int): The flash index where the image will be stored (0-255).
        Returns:
            bool: True if the image was successfully loaded, False otherwise.'''

        self.openMailbox(function=1)  # Function 1: Flash image configuration
        self.setLUTOffsetPointer(offset=flash_index) # Set the LUT offset pointer to the desired flash index
        # The image data must be split into chunks of 64 bytes (HID packet size)
        chunk_size = 64
        for i in range(0, len(image_data), chunk_size):
            chunk = image_data[i:i + chunk_size]
            # Convert the map object to a list before writing
            pkt = list(self.buildPacket(0x1A, 0x34, data=list(chunk), readonly=0, reply=0))
            self.dlp_hid.write(pkt)
            if self.debug:
                print(f"Sent chunk {i // chunk_size + 1} of {len(image_data) // chunk_size + 1}")

        self.closeMailboxes()
        flash_status = self.getFlashStatus()
        if flash_status is not None:
            print(f"Image loaded successfully into flash index {flash_index}.")
            return True
        else:
            print("Error: Failed to load image into flash memory.")
            return False
        
    
    def loadInternalTestPattern(self, pattern):
        """Internal Test Patterns Select (CMD2: 0x12, CMD3: 0x03)
        Selects the internal test pattern to be displayed.
        """
        pkt = self.buildPacket(0x12, 0x03, data=[pattern], readonly=0, reply=0)
        self.dlp_hid.write(pkt)
        if self.debug:
            self.dumpPacket(pkt, [], '(Set Internal Test Pattern)')
        return 1
