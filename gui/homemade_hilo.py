...
def create_patterned_image(image_path, output_path, square_size=10, inverted=False):
    """
    Loads an image, resizes it, and overlays a checkerboard pattern where some areas 
    remain visible while others are black.
    
    :param image_path: Path to the input image
    :param output_path: Path to save the output image with checkerboard pattern
    :param size: Target size for resizing (default: 912x1140)
    :param square_size: Size of each checkerboard square (default: 50 pixels)
    """
    ...
    if inverted==True:
        mask = Image.new("L", (width, height), 0)  # 255 means fully visible
    else:
        mask = Image.new("L", (width, height), 255)  # 255 means fully visible
    ...
    for y in range(0, height, square_size * 2):
        for x in range(0, width, square_size * 2):
            if inverted==True:
                draw.rectangle([x, y, x + square_size - 1, y + square_size - 1], fill=255)  # Black square
                draw.rectangle([x + square_size, y + square_size, x + 2 * square_size - 1, y + 2 * square_size - 1], fill=255)
            else:
                draw.rectangle([x, y, x + square_size - 1, y + square_size - 1], fill=0)  # Black square
...
def hilo_reconstruction_contrasted(I_high, I_1, I_2, alpha=1.0, sigma=5, contrast_threshold=0.5):
    """
    HiLo image reconstruction with improved low-frequency extraction.
    ...
    - contrast_threshold: Threshold to keep in-focus regions based on local contrast.
    ...
    # Compute the low-frequency component as the average of checkerboard images
    # I_low = (I_1 * mask1.astype(np.float32) + I_2 * mask2.astype(np.float32))  # Averaging method

    # Weighted averaging instead of binary thresholding
    ...
    # Combine low and high-freq components and normalize
